import type { ClientJiraConfig, ResolvedJiraConfig } from "./clientConfig.js";
import { legacyGlobalJiraConfig, resolveJiraConfig } from "./clientConfig.js";
...
type JiraRuntime = {
  ...
  issueCache: Map<string, CacheEntry<JiraIssue>>;
  cloudIdCache: CacheEntry<string> | null;
  issuesListCache: CacheEntry<JiraIssue[]> | null;
};

const runtimes = new Map<string, JiraRuntime>();

function jiraConfiguredFromConfig(config: ClientJiraConfig | undefined): config is ClientJiraConfig {
...
export function jiraStatusFromConfig(config: ClientJiraConfig | undefined) {
  const configured = jiraConfiguredFromConfig(config);
  return {
    configured,
...
async function searchIssues(runtime: JiraRuntime, jql: string, maxResults = 100): Promise<JiraIssue[]> {
...
function getCachedIssue(runtime: JiraRuntime, key: string): JiraIssue | null {
...
function setCachedIssue(runtime: JiraRuntime, issue: JiraIssue) {
...
async function fetchMyIssuesForRuntime(runtime: JiraRuntime): Promise<JiraIssue[]> {
...
async function fetchIssueMapForRuntime(runtime: JiraRuntime, keys: string[]): Promise<Record<string, JiraIssue>> {
...
export async function fetchMyIssues(clientKey?: string | null): Promise<JiraIssue[]> {
  const resolved = await resolveJiraConfig(clientKey);
  if (!resolved) {
    return [];
  }
...
export async function fetchIssueMap(keys: string[], clientKey?: string | null): Promise<Record<string, JiraIssue>> {
  const resolved = await resolveJiraConfig(clientKey);
  if (!resolved) {
    return {};
  }
...
// Backwards-compatible helpers for legacy global Jira env when no client registry exists.
export function jiraStatus() {
  return legacyJiraStatus();
}

export async function fetchMyIssuesLegacy(): Promise<JiraIssue[]> {
  return fetchMyIssues(null);
}

export async function fetchIssueMapLegacy(keys: string[]): Promise<Record<string, JiraIssue>> {
  return fetchIssueMap(keys, null);
}
